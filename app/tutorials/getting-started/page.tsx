...
function Page() {
    ...
    <>
      ...
      <article className="mt-8 w-full min-w-0 max-w-6xl px-1 md:px-6 text-left">
        ...
        <Content>
          ...
          를 통해 프로젝트 생성을 원하는 디렉토리로 이동합니다. 이동한 후 아래와
          같이 입력합니다.
        </Content>
        <CodeBlock
          route={">_ Terminal"}
        ...
        <Content>
          ...
          를 사용합니다. 위의 명령어에서{" "}
          <CodeWithNoLink>--example</CodeWithNoLink> 플래그는 해당 코스를 위한
          소스 코드를
          <CodeWithLink
            isCode={false}
        ...
        <ContentTitle title="프로젝트 살펴보기" id="exploring-the-project" />
        <Content>
          코드를 처음부터 작성해야하는 다른 튜토리얼과는 다르게, 대부분의 코드는
          이미 당신을 위해 작성되어 있습니다. 이것은 이미 코드 베이스가 있는
          곳에서 일하게 될 실제 개발 환경을 잘 반영하고 있습니다.
        </Content>
        <Content>
          우리의 목표는 Next.js의 핵심 기능을 배우는 것이기 때문에, 모든 코드를
          다 작성할 필요는 없습니다.
        </Content>
        <Content>
        ...
        <ul className="list-disc pl-8 flex flex-col gap-4 my-5">
          ...
          <li>
            <CodeWithNoLink>/app/lib</CodeWithNoLink> : 어플리케이션에서 사용될
            함수들이 담겨져있습니다. 예를들어 재사용 가능한 함수나 데이터를
            가져오는 함수들이 들어있습니다.
          </li>
          <li>
          ...
          <li>
            ...
            데이터베이스를 채우는 데 사용될 시드 스크립트가 포함되어 있습니다.
          </li>
          <li>
            <b>Config Files</b>: 아마{" "}
            <CodeWithNoLink>next.config.js</CodeWithNoLink>와 같은 설정 파일들이
            어플리케이션의 루트에 있음을 알 수 있습니다. 대부분의 파일들은{" "}
            <CodeWithNoLink>create-next-app</CodeWithNoLink>을 사용하면서 미리
        ...
        <Content>
          유저 인터페이스를 만들 때, 몇 개의 placeholder 데이터를 가지는 것을
        ...
        <Content>
          ...
          placeholder 데이터를 제공하고 있습니다. 각각의 Javascript 객체는
          당신의 데이터베이스의 테이블을 의미합니다. 예를 들어 invoices
          테이블에서
        </Content>
        <CodeBlock
  ...
  // ...
        ...
        <Content>
          ...
          <CodeWithNoLink>.tsx</CodeWithNoLink>로 끝나는 것을 볼 수 있습니다.
          우리는 이 코스가 최근 웹 생태계를 반영할 수 있기를 원했습니다.
        </Content>
        <Content>
          Typescript에 대해서 몰라도 괜찮습니다. - 저희가 Typescript가 필요한
          부분의 코드는 제공할 것입니다.
        </Content>
        <Content>
          이제 <CodeWithNoLink>/app/lib/definitions.ts</CodeWithNoLink> 파일을
          한 번 보시길 바랍니다. 여기서 우리는 데이터베이스에서 받을
          Typescript의 타입들을 정의하고 있습니다. 예를 들어 invoices 테이블은
          아래와 같은 타입을 가집니다.
        </Content>
        <CodeBlock
  ...
  status: 'pending' | 'paid';
        ...
        <Content>
          ...
          포맷의 데이터를 전달하는 것을 방지할 수 있습니다. 예를 들어 amount에
          number가 아닌 string을 전달하려고 하면, Typescript는 에러를
          발생시킵니다.
        </Content>
        <ContentTitle
          title="개발 서버 실행시키기"
        ...
        />
        <Content>
          <CodeWithNoLink>npm run dev</CodeWithNoLink>를 실행시켜서 개발 서버를
          실행시켜줍니다.
        </Content>
        <CodeBlock
          route=">_ Terminal"
        ...
        <div className="relative w-full h-[150px] p-4 bg-vercel-200 rounded-xl">
          <Image src={"/acme-unstyled.png"} alt="acme-unstyled" fill={true} />
        </div>
        <Complete
        ...
        >
          축하드립니다! 스타터 예제를 통해서 Next.js 어플리케이션을 만들고 개발
          서버에 실행까지 완료했습니다.
        </Complete>
      </article>
    </>
